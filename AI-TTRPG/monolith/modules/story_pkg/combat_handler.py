...
logger = logging.getLogger("monolith.story.combat")# --- Combat Reward Helper ---def _grant_combat_rewards(db: Session, combat: models.CombatEncounter, log: List[str]):
"""
...
# 4. Roll on loot table (Simple implementation)
if loot_table_ref == "goblin_easy":
if random.random() < 0.75: # 75% chance to drop scrap
item_to_drop = "item_scrap_metal"
services.add_item_to_character(primary_player_id, item_to_drop, 1)
log.append(f"The {template_id} dropped {item_to_drop}!")

# TODO: Add XP rewards

except Exception as e:
logger.exception(f"Failed to grant loot for {p.actor_id}: {e}")# --- Combat Logic Helpers ---def _find_spawn_points(map_data: List[List[int]], num_points: int) -> List[List[int]]:
if not map_data:
...
return [valid_spawns[i % len(valid_spawns)] for i in range(num_points)]def _extract_initiative_stats(stats_dict: Dict) -> Dict:
...
abilities=npc_abilities # Pass abilities to spawn
)
...
return db_combatdef get_actor_context(actor_id: str) -> Tuple[str, Dict]:
...
raise HTTPException(status_code=400, detail=f"Unknown actor ID format: {actor_id}")def get_stat_score(actor_context: Dict, stat_name: str) -> int:
stats = actor_context.get("stats", {})
return stats.get(stat_name, 10)def get_skill_rank(actor_context: Dict, skill_name: str) -> int:
...
return "Unarmed/Fist Weapons", "melee"def get_equipped_armor(actor_context: Dict) -> Optional[str]:
actor_name = actor_context.get('name', actor_context.get('template_id', 'Unknown Actor'))
equipment = actor_context.get("equipment")
if equipment is not None: # Player
...
return "Natural/Unarmored"def check_combat_end_condition(db: Session, combat: models.CombatEncounter, log: List[str]) -> bool:
...
return True
return Falsedef determine_npc_action(db: Session, combat: models.CombatEncounter, npc_actor_id: str) -> Optional[schemas.PlayerActionRequest]:
"""
...
services.remove_status_from_target(npc_actor_id, "Staggered")
...
return schemas.PlayerActionRequest(action="attack", target_id=target_id)def handle_no_action(db: Session, combat: models.CombatEncounter, actor_id: str, reason: str = "waits") -> schemas.PlayerActionResponse:
...
)# --- NEW: Dice Rolling Helper ---def _roll_dice_string(dice_str: str) -> int:
...
return 0# --- NEW: Ability Effect Handlers ---# These are the small helper functions that execute our data-driven effects.def _handle_effect_modify_attack(
actor_id: str,
target_id: str,
...
"""
return _handle_basic_attack(actor_id, target_id, attacker_context, defender_context, log, ability_mod=effect)def _handle_effect_direct_damage(
target_id: str,
log: List[str],
effect: Dict) -> bool:
"""
...
return False
return Truedef _handle_effect_heal(
target_id: str,
log: List[str],
effect: Dict) -> bool:
"""
...
return Truedef _handle_effect_apply_status(
target_id: str,
log: List[str],
effect: Dict) -> bool:
"""
...
return Truedef _handle_effect_apply_status_roll(
target_id: str,
log: List[str],
effect: Dict) -> bool:
"""
...
stat_mod = services.calculate_modifier(stat_score)
...
services.apply_status_to_target(target_id, status_id)
return Truedef _handle_effect_move_target(
target_id: str,
log: List[str],
effect: Dict) -> bool:
"""
...
return False# This is the "router" that maps `effect["type"]` to the functions above
...
}# --- Main Attack Handler ---def _handle_basic_attack(
actor_id: str,
target_id: str,
attacker_context: Dict,
defender_context: Dict,
...
return False# --- Main Action Handler ---def handle_player_action(db: Session, combat: models.CombatEncounter, actor_id: str, action: schemas.PlayerActionRequest) -> schemas.PlayerActionResponse:
log = []
...
services.remove_status_from_target(actor_id, "Staggered")
