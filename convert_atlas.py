...
xml_content = """<TextureAtlas imagePath="sheet.png">
...
# We renamed the image to uipack_rpg_sheet.png
image_filename = "uipack_rpg_sheet.png"

atlas_data = {
    image_filename: {}
}

# Image dimensions (from check_image_size.py output)
IMAGE_HEIGHT = 512 # Validated from typical spritesheet sizes, but will confirm with command output

for sub in root.findall('SubTexture'):
    name = sub.get('name').replace('.png', '') # Remove extension for cleaner IDs
    x = int(sub.get('x'))
    y_xml = int(sub.get('y'))
    w = int(sub.get('width'))
    h = int(sub.get('height'))
    
    # Kivy Atlas format: "id": [x, y, w, h]
    # Kivy uses bottom-left origin, XML usually uses top-left.
    # y_kivy = IMAGE_HEIGHT - y_xml - h
    y_kivy = IMAGE_HEIGHT - y_xml - h
    
    atlas_data[image_filename][name] = [x, y_kivy, w, h]

output_path = r"c:\Users\krazy\Documents\GitHub\ShatterlandsTTRPGAI\game_client\assets\graphics\ui\uipack_rpg.atlas"
