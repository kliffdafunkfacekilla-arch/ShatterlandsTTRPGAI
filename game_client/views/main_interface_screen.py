...
"""import loggingfrom kivy.app import Appfrom kivy.lang import Builderfrom kivy.uix.screenmanager import Screenfrom kivy.uix.floatlayout import FloatLayoutfrom kivy.uix.boxlayout import BoxLayoutfrom kivy.uix.anchorlayout import AnchorLayoutfrom kivy.uix.scrollview import ScrollViewfrom kivy.uix.label import Labelfrom kivy.uix.textinput import TextInputfrom kivy.uix.button import Buttonfrom kivy.properties import ObjectProperty, ListProperty, StringPropertyfrom kivy.core.window import Windowfrom functools import partial# --- Monolith Imports ---try:
...
from monolith.modules.story_pkg import schemas as story_schemasexcept ImportError as e:
...
character_api, story_api, story_schemas = None, None, None# --- Client Imports ---try:
from game_client import asset_loader
from game_client.views.map_view_widget import MapViewWidget # <-- IMPORT OUR NEW WIDGETexcept ImportError as e:
logging.error(f"MAIN_INTERFACE: Failed to import client modules: {e}")
asset_loader = None
MapViewWidget = None# Constants
TILE_SIZE = 64 # Must match the MapViewWidget's TILE_SIZE# --- Kivy Language (KV) String for the UI Layout ---# This is *almost* the same as before, but note the change in the 'CENTER' section
MAIN_INTERFACE_KV = """
<MainInterfaceScreen>:
...
# This is now a container that will *hold* our MapViewWidget
# We give it an ID so our Python code can find it
BoxLayout:
id: map_view_anchor
size_hint: None, None
# The MapViewWidget's size will be set in build_scene()

# --- (All other UI panels: Log, Party, Narration, Menu Bar... are unchanged) ---
...
# --- NEW: Reference to our Map Widget ---
...
# --- Add the MapViewWidget to the anchor ---
# We do this here to ensure 'ids' are populated
self.map_view_anchor = self.ids.get('map_view_anchor')
...
# ... (rest of on_enter: load app, char_name, check modules) ...
...
"""
Tells the MapViewWidget to build itself.
"""
...
# Update the anchor's size to match the map
...
# --- Game Logic Methods (Refactored) ---

def get_map_height(self):
"""Helper to get the map height."""
...
"""Calls monolith to move player and updates the map widget."""
...
# --- Tell the Map Widget to move the sprite ---
...
# --- Mouse Control / Interaction Logic (Unchanged from previous step) ---
...
actor_id=actor_id, location_id=loc_id,
...
self.build_scene() # Re-render map for changes
except Exception as e:
...
# We now check against the widget, not the anchor
