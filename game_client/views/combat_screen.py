...
"""import loggingfrom kivy.app import Appfrom kivy.uix.screenmanager import Screenfrom kivy.uix.label import Labelfrom kivy.uix.boxlayout import BoxLayoutfrom kivy.uix.button import Buttonfrom kivy.uix.scrollview import ScrollViewfrom kivy.uix.anchorlayout import AnchorLayoutfrom kivy.properties import ObjectProperty, StringProperty, BooleanProperty, ListProperty, DictPropertyfrom kivy.clock import Clockfrom functools import partialfrom kivy.core.window import Windowfrom typing import Optional, List# --- Monolith Imports ---try:
...
from monolith.modules.story_pkg import services as story_services # <-- Import story servicesexcept ImportError as e:
...
story_services = None# --- Client Imports ---try:
...
TILE_SIZE = 64 # Must match MapViewWidgetclass CombatScreen(Screen):
...
db_char = char_crud.get_character(char_db, char_ctx.id.split('_')[-1]) # Get by UUID
...
if self.current_action == "use_ability" and self.selected_ability_id and story_services:
ability_data = story_services.get_ability_data(self.selected_ability_id)
ability_target_type = ability_data.get("target_type", "enemy")
if ability_target_type in ("ally", "self"):
is_friendly_action = True
